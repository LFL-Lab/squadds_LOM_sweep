...
def cross_LOM_sweep(design, sweep_opts):
    for param in extract_QSweep_parameters(sweep_opts):
        ...
        # example: update single setting
        loma.sim.setup.max_passes = 30
        loma.sim.setup.min_converged_passes = 2
        ...
        data_df = {
            ...
            "sim_info": {
                "setup": setup,
                "renderer_options": loma.sim.renderer.options
            },
            "misc" : data
        }

        filename = f"cross_LOM_groundspacing{cross.options.connection_pads.readout.ground_spacing}_crosslen{cross.options.cross_length}_clawlen{cross.options.connection_pads.readout.claw_length}"
        save_simulation_data_to_json(data_df, filename)
